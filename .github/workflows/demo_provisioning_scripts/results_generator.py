...
def load_scenario_json(filename):
    """Load scenario data from JSON file"""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    repo_root = os.path.dirname(os.path.dirname(script_dir))
    json_path = os.path.join(repo_root, "data", "scenarios", filename)
...
def generate_deterministic_user(seed_key):
    """Generate a deterministic user for consistent demo data"""
    random.seed(seed_key)
    user_key = f"demo-user-{seed_key}"
    builder = Context.builder(user_key)
    builder.set("name", f"Demo User {seed_key}")
    builder.set("email", f"demo-{seed_key}@togglebank.com")
    builder.set("accountType", "personal")
    builder.set("segment", "beta-users")
    random.seed()
    return builder.build()

def fraud_detection_healthy_scenario_generator(client):
    """Generate metrics for healthy fraud detection rollout"""
    if not client.is_initialized():
        logging.error("LaunchDarkly client is not initialized for Fraud Detection Healthy scenario")
        return
    
    logging.info("Starting Fraud Detection Healthy scenario generator...")
    scenario_data = load_scenario_json("fraud-healthy.json")
    
    if not scenario_data:
        logging.error("Failed to load fraud-healthy.json")
        return
    ...
    for event in timeline:
        if "metrics" in event:
            ...
            users_in_rollout = int(num_users * percentage / 100)
            
            for i in range(users_in_rollout):
                try:
                    user = generate_deterministic_user(f"fraud-healthy-{i}")
                    
                    error_rate = metrics.get("errorRate", 0.02)
                    if random.random() < (error_rate / 100):
                        client.track("fraud-api-error-rate", user)
                    
                    latency = int(metrics.get("latency", 145) + random.uniform(-5, 5))
                    client.track("fraud-api-latency", user, None, latency)
                    
                    success_rate = metrics.get("successRate", 99.98)
                    if random.random() < (success_rate / 100):
                        client.track("fraud-check-success-rate", user)
                
                except Exception as e:
                    logging.error(f"Error generating fraud healthy metrics: {str(e)}")
                    continue
            
            logging.info(f"Generated fraud healthy metrics for {percentage}% rollout")
    
    logging.info("Fraud Detection Healthy scenario completed")

def fraud_detection_failed_scenario_generator(client):
    """Generate metrics for failed fraud detection rollout with spike and recovery"""
    if not client.is_initialized():
        logging.error("LaunchDarkly client is not initialized for Fraud Detection Failed scenario")
        return
    
    logging.info("Starting Fraud Detection Failed scenario generator...")
    scenario_data = load_scenario_json("fraud-failed.json")
    
    if not scenario_data:
        logging.error("Failed to load fraud-failed.json")
        return
    ...
    for event in timeline:
        if "metrics" in event:
            ...
            users_in_rollout = int(num_users * percentage / 100) if percentage > 0 else 200
            
            for i in range(users_in_rollout):
                try:
                    user = generate_deterministic_user(f"fraud-failed-{i}-{percentage}")
                    
                    error_rate = metrics.get("errorRate", 0.02)
                    if random.random() < (error_rate / 100):
                        client.track("fraud-api-error-rate", user)
                    
                    latency = int(metrics.get("latency", 145) + random.uniform(-10, 10))
                    client.track("fraud-api-latency", user, None, latency)
                    ...
                    logging.error(f"Error generating fraud failed metrics: {str(e)}")
                    continue
            
            logging.info(f"Generated fraud failed metrics for {percentage}% rollout (status: {status})")
    
    logging.info("Fraud Detection Failed scenario completed")

def notification_center_healthy_scenario_generator(client):
    """Generate metrics for healthy notification center rollout"""
    if not client.is_initialized():
        logging.error("LaunchDarkly client is not initialized for Notification Center Healthy scenario")
        return
    
    logging.info("Starting Notification Center Healthy scenario generator...")
    scenario_data = load_scenario_json("notifications-healthy.json")
    
    if not scenario_data:
        logging.error("Failed to load notifications-healthy.json")
        return
    ...
    for event in timeline:
        if "metrics" in event:
            ...
            for i in range(users_in_rollout):
                try:
                    user = generate_deterministic_user(f"notif-healthy-{i}")
                    
                    notif_volume = int(metrics.get("notificationVolume", 250))
                    for _ in range(notif_volume):
                        client.track("notification-event-volume", user)
                    
                    client_errors = metrics.get("clientErrors", 0.1)
                    if random.random() < (client_errors / 100):
                        client.track("notification-client-errors", user)
                    
                    display_latency = int(metrics.get("displayLatency", 85) + random.uniform(-5, 5))
                    client.track("notification-display-latency", user, None, display_latency)
                
                except Exception as e:
                    logging.error(f"Error generating notification healthy metrics: {str(e)}")
                    continue
            
            logging.info(f"Generated notification healthy metrics for {percentage}% rollout")
    
    logging.info("Notification Center Healthy scenario completed")

def notification_center_failed_scenario_generator(client):
    """Generate metrics for failed notification center rollout with spam spike"""
    if not client.is_initialized():
        logging.error("LaunchDarkly client is not initialized for Notification Center Failed scenario")
        return
    
    logging.info("Starting Notification Center Failed scenario generator...")
    scenario_data = load_scenario_json("notifications-failed.json")
    
    if not scenario_data:
        logging.error("Failed to load notifications-failed.json")
        return
    ...
    for event in timeline:
        if "metrics" in event:
            ...
            for i in range(users_in_rollout):
                try:
                    user = generate_deterministic_user(f"notif-failed-{i}-{percentage}")
                    ...
                    display_latency = int(metrics.get("displayLatency", 85) + random.uniform(-10, 10))
                    client.track("notification-display-latency", user, None, display_latency)
                
                except Exception as e:
                    logging.error(f"Error generating notification failed metrics: {str(e)}")
                    continue
            
            logging.info(f"Generated notification failed metrics for {percentage}% rollout (status: {status})")
    
    logging.info("Notification Center Failed scenario completed")
...
def generate_results(project_key, api_key):
    ...
    if sdk_key:
        ...
        # Scenario-based static data generators
        fraud_detection_healthy_scenario_generator(client)  # fraudDetectionHealthyRollout
        fraud_detection_failed_scenario_generator(client)  # fraudDetectionFailedRollout
        notification_center_healthy_scenario_generator(client)  # notificationCenterHealthyRollout
        notification_center_failed_scenario_generator(client)  # notificationCenterFailedRollout

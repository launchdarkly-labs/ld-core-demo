...
export default function SuggestedItems({ cart, setCart }: { cart: any; setCart: any }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-full my-4  font-sohne ">
      <div className="flex flex-col items-center bg-gradient-experimentation text-transparent bg-clip-text my-4">
        Suggested Items
      </div>
        ...
        {VR_GALAXY_DATA?.slice(5, 8)?.map((item) => (
          <div key={item.id} className="flex flex-col items-center">
            ...
            <div className="flex flex-col items-center ">
              <p className="legend mb-12">
                {item.item} - ${item.cost}
              </p>
              <button
                className="legend bg-gradient-experimentation"
                onClick={() => addedSuggestedItemToCart(item)}
              >
                Add To Cart
              </button>
            </div>

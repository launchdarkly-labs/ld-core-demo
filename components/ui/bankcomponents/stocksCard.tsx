...
import { IoIosArrowDropupCircle, IoIosArrowDropdownCircle, IoMdRemoveCircleOutline } from "react-icons/io";
...
export const StocksComponent: React.FC = () => {
    ...
    const generateChartData = (stockData) => {
        ...
        const secondLastValue = lastTenDataPoints.length > 1 ? lastTenDataPoints[lastTenDataPoints.length - 2].value : lastTenDataPoints[0].value;
        const lastValue = lastTenDataPoints[lastTenDataPoints.length - 1].value;
        const directionColor = lastValue > secondLastValue ? 'lightgreen' : lastValue < secondLastValue ? 'red' : 'rgba(255, 255, 255, 0.5)';

...
export default function RollbackToggleBankAPIRelease() {
  ...
  const startGeneration = useCallback(async () => {
    ...
    const metrics = [
      { key: "stocks-api-latency", type: "event" },
      { key: "stocks-api-error-rates", type: "occurrence" },
    ];
    const featureFlagKey = "togglebankAPIGuardedRelease";
    ...
    try {
      ...
      while (trueVariationContexts.length < 100 && !shouldStop) {
        const userContexts = Array.from({ length: 1000 }, (_, i) => ({
          key: uuidv4(),
        }));
        ...
        for (const context of userContexts) {
          ...
          const variation = await ldClient?.variation(featureFlagKey, false);
          if (variation) {
            trueVariationContexts.push(context);
            console.log(
              "Length of trueVariationContexts: ",
              trueVariationContexts.length
            );
          } else {
        ...
        for (const context of contexts) {
          ...
          for (const metric of metrics) {
            ...
            if (metric.type === "event") {
              await ldClient?.track(metric.key, latencyNumber);
            } else {
      ...
      await processMetrics(trueVariationContexts, { min: 300, max: 400 });
      await processMetrics(falseVariationContexts, { min: 250, max: 300 });

      if (!shouldStop) {
        setTimeout(() => {
          setProgress(100);
          setCompletionMessage("Process completed successfully!");
        }, 100);
      }
    } catch (error) {
    ...
    <Dialog>
      <DialogTrigger asChild>
        <p className="font-bold font-sohnelight text-lg">
          Rollback Toggle Bank API Release
        </p>
      </DialogTrigger>
      <DialogContent>
        {isGenerating ? (
          ...
          <div className="flex justify-center text-xl font-bold items-center h-full">
            {completionMessage && (
              <p className="text-green-500 mb-4">{completionMessage}</p>
            )}
            <button
            ...
            >
              Start Rollback Toggle Bank API Release
            </button>
          </div>
        )}
      </DialogContent>
